...
const migrateConfig = {
  ...
  '183': (state: RootState) => {
    try {
      // Add 'hosts' to sidebar icons if not present
      if (state.settings && state.settings.sidebarIcons) {
        if (
          !state.settings.sidebarIcons.visible.includes('hosts') &&
          !state.settings.sidebarIcons.disabled.includes('hosts')
        ) {
          // Insert 'hosts' after 'assistants' in visible icons
          const assistantsIndex = state.settings.sidebarIcons.visible.indexOf('assistants')
          if (assistantsIndex !== -1) {
            state.settings.sidebarIcons.visible.splice(assistantsIndex + 1, 0, 'hosts')
          } else {
            // If 'assistants' not found, add at the beginning
            state.settings.sidebarIcons.visible.unshift('hosts')
          }
        }
      }
