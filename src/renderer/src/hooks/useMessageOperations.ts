 ...
 */
export function useMessageOperations(topic: Topic | undefined | null) {
  ...
  const editMessage = useCallback(
    async (messageId: string, updates: Partial<Omit<Message, 'id' | 'topicId' | 'blocks'>>) => {
      ...
      // Call the thunk with topicId and only message updates
  ...
  const resendUserMessageWithEdit = useCallback(
    async (message: Message, editedBlocks: MessageBlock[], assistant: Assistant) => {
      ...
      if (!topicId) return
      await dispatch(newMessagesActions.updateMessage({ topicId, messageId: message.id, updates: messageUpdates }))
