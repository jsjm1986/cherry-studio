...
import { addTopic, updateTopic } from '@renderer/store/assistants'
...
export function useActiveTopic(assistantId: string, topic?: Topic) {
  ...
  // 当助手没有话题时，自动创建一个默认话题
  useEffect(() => {
    if (assistant && assistant.topics && Array.isArray(assistant.topics) && assistant.topics.length === 0) {
      const newTopic = getDefaultTopic(assistant.id)
      db.topics.add({ id: newTopic.id, messages: [] }).then(() => {
