...
export type Assistant = {
  ...
  handle?: string
  /** 触发关键词，用于 @提及时的搜索匹配 (仅 type='expert' 时使用) */
...
/** 专家提示词设置 */
export type ExpertPromptSettings = {
  /** 是否启用提示词增强模式（添加身份强化指令） */
  enableEnhancedMode?: boolean
  /** 主机提示词处理方式: 'ignore' 忽略, 'append' 附加到专家提示词后 */
...
/** 判断是否为主机 */
export const isHost = (assistant: Assistant): assistant is Host => {
  return assistant.type === 'host'
}

/** 判断是否为专家 */
export const isExpert = (assistant: Assistant): assistant is Expert => {
  return assistant.type === 'expert' && typeof assistant.hostId === 'string'
}
