...
const AssistantModelSettings: FC<Props> = ({ assistant, updateAssistant, updateAssistantSettings }) => {
  ...
  // 当 assistant prop 变化时，同步本地状态
  useEffect(() => {
    setTemperature(assistant?.settings?.temperature ?? DEFAULT_TEMPERATURE)
    setContextCount(assistant?.settings?.contextCount ?? DEFAULT_CONTEXTCOUNT)
    setEnableMaxTokens(assistant?.settings?.enableMaxTokens ?? false)
    setMaxTokens(assistant?.settings?.maxTokens ?? 0)
    setStreamOutput(assistant?.settings?.streamOutput)
    setToolUseMode(assistant?.settings?.toolUseMode ?? 'function')
    setDefaultModel(assistant?.defaultModel)
    setTopP(assistant?.settings?.topP ?? 1)
    setEnableTopP(assistant?.settings?.enableTopP ?? false)
    setCustomParameters(assistant?.settings?.customParameters ?? [])
