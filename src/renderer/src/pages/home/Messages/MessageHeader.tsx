...
const MessageHeader: FC<Props> = memo(({ assistant, model, message, topic, isGroupContextMessage }) => {
  ...
  // 检查消息是否来自专家（Hosts 功能）
  const isExpertMessage = Boolean(message.expertId)
  const expertEmoji = message.expertEmoji
  const expertName = message.expertName
  ...
  const getUserName = useCallback(() => {
    ...
    // 如果是专家消息，显示专家名称
    if (message.role === 'assistant' && isExpertMessage && expertName) {
      return expertName
    }
  ...
  }, [agent?.name, isAgentView, message, model, t, userName, isExpertMessage, expertName])
  ...
  return (
    <Container className="message-header">
      {isAssistantMessage ? (
        // 如果是专家消息，显示专家 emoji 头像
        isExpertMessage && expertEmoji ? (
